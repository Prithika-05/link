...
import RestoreKeyModal from "../components/common/RestoreKeyModal";
...
import { getStoredKeyPair } from "../services/cryptoService";
...
export default function AppLayout({ children }) {
  ...
  // Check if current device has the private key stored in IndexedDB
  useEffect(() => {
  ...
  const handleRestoreSuccess = () => {
    ...
    // Reload page to re-trigger conversation decryption with the restored private key
  ...
  return (
    <div className="app-shell">
      <aside className="side-nav">
        ...
        <nav className="side-nav-links" aria-label="Main navigation">
          {navigationItems.map((item) => (
            ...
            >
              ...
              {item.label === "Chats" && unreadTotal > 0 && (
                <b>{unreadTotal}</b>
              )}
            </NavLink>
          ))}
        ...
        <div className="side-nav-bottom">
          <div
            ...
            title={`REST: ${backendStatus}; realtime: ${socketStatus}`}
          >
      ...
      {/* Triggered automatically if IndexedDB lacks private key */}
      <RestoreKeyModal
        isOpen={showRestoreModal}
        onSuccess={handleRestoreSuccess}
        onResetNewKey={() => navigate("/key-setup")}
      />
    </div>
  );
}
