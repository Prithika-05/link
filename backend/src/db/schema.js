...
// ENUMS
// ==========================================
export const userStatusEnum = pgEnum('UserStatus', ['ONLINE', 'OFFLINE', 'AWAY']);
export const messageStatusEnum = pgEnum('MessageStatus', ['SENT', 'DELIVERED', 'READ', 'FAILED']);
export const messageTypeEnum = pgEnum('MessageType', ['TEXT', 'IMAGE', 'FILE', 'SYSTEM']);
...
export const users = pgTable('User', {
  ...
  publicId: text('publicId').notNull().unique().$defaultFn(() => createId()),
...
export const publicKeys = pgTable('PublicKey', {
  id: text('id').primaryKey().$defaultFn(() => createId()),
  ...
  index('PublicKey_userId_idx').on(table.userId),
]);
...
export const messages = pgTable('Message', {
  ...
  index('Message_conversation_idx').on(table.senderId, table.receiverId, table.createdAt),
]);
...
export const deviceSessions = pgTable('DeviceSession', {
  id: text('id').primaryKey().$defaultFn(() => createId()),
  userId: text('userId').notNull().references(() => users.id, { onDelete: 'cascade' }),
  refreshTokenId: text('refreshTokenId').notNull().unique().references(() => refreshTokens.id, { onDelete: 'cascade' }),
...
export const revokedTokens = pgTable('RevokedToken', {
  ...
  index('RevokedToken_expiresAt_idx').on(table.expiresAt),
]);
